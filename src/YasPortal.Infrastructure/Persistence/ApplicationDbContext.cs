...
public sealed class ApplicationDbContext(DbContextOptions<ApplicationDbContext> options, CurrentUser? currentUser = null) : DbContext(options), IApplicationDbContext
{
    ...
    private static string? GetEntityId(EntityEntry entry)
    {
        ...
        return string.Join(",", keys.Select(p => $"{p.Metadata.Name}={p.CurrentValue ?? p.OriginalValue}"));
    }
    ...
    private void ValidateEmployeeAssignments()
    {
        ...
        foreach (var entry in activeEntries)
        {
            ...
            var localConflict = activeEntries.Any(other => !ReferenceEquals(other, entry) && other.Entity.PositionId == entry.Entity.PositionId && other.Entity.EmployeeId != entry.Entity.EmployeeId && !IsBeingEnded(other));
            if (localConflict) throw new InvalidOperationException("A position can have only one active employee.");
    ...
    private async Task ValidateEmployeeAssignmentsAsync(CancellationToken cancellationToken)
    {
        ...
        var activeEntries = ChangeTracker.Entries<EmployeePosition>()
            .Where(x => x.State == EntityState.Added || (x.State == EntityState.Modified && x.Property(p => p.EndedAt).IsModified && x.Property(p => p.EndedAt).CurrentValue is null))
            .ToList();
    ...
    private void ValidateActiveEmployee(Guid employeeId)
    {
        ...
        {
            if (!localEmployee.IsActive) throw new InvalidOperationException("An inactive employee cannot have an active position assignment.");
            return;
        }
    ...
    private void ValidatePositionHierarchy()
    {
        ...
        ApplyTrackedPositionChanges(parents); ValidateHierarchyGraph(parents);
    }
    ...
    private void ApplyTrackedPositionChanges(Dictionary<Guid, Guid?> parents)
    {
        ...
        {
            if (entry.State == EntityState.Deleted) { parents.Remove(entry.Entity.Id); continue; }
    ...
    private static void ValidateHierarchyGraph(IReadOnlyDictionary<Guid, Guid?> parents)
    {
        ...
        {
            var visited = new HashSet<Guid>(); var current = (Guid?)start;
            while (current is Guid positionId)
            {
                if (!visited.Add(positionId)) throw new InvalidOperationException("The position hierarchy contains a cycle.");
                if (!parents.TryGetValue(positionId, out current)) break;
            }
        }
    }
    ...
    private void CaptureAssignmentHistory()
    {
        ...
        foreach (var entry in entries.Where(x => x.State == EntityState.Modified && x.Property(p => p.EndedAt).IsModified))
        {
            ...
            {
                var activeHistory = PositionAssignmentHistories.Local.FirstOrDefault(x => x.PositionId == entry.Entity.PositionId && x.EndedAt is null)
                    ?? PositionAssignmentHistories.FirstOrDefault(x => x.PositionId == entry.Entity.PositionId && x.EndedAt == null);
    ...
    private async Task CaptureAssignmentHistoryAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in entries.Where(x => x.State == EntityState.Modified && x.Property(p => p.EndedAt).IsModified))
        {
            ...
            {
                var activeHistory = PositionAssignmentHistories.Local.FirstOrDefault(x => x.PositionId == entry.Entity.PositionId && x.EndedAt is null)
                    ?? await PositionAssignmentHistories.FirstOrDefaultAsync(x => x.PositionId == entry.Entity.PositionId && x.EndedAt == null, cancellationToken);
                activeHistory?.End(currentEndedAt.Value);
            }
            else if (originalEndedAt is not null && currentEndedAt is null && !PositionAssignmentHistories.Local.Any(x => x.EmployeeId == entry.Entity.EmployeeId && x.PositionId == entry.Entity.PositionId && x.EndedAt is null))
                PositionAssignmentHistories.Add(new PositionAssignmentHistory(entry.Entity.EmployeeId, entry.Entity.PositionId));
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<AuditEntry>(e =>
        {
            ...
            e.Property(x => x.EntityId).HasMaxLength(500);
