...
public sealed class AdminQueryService(IDbContextFactory<ApplicationDbContext> dbFactory)
{
    ...
    public async Task<EmployeeSummary?> GetEmployeeAsync(Guid id, CancellationToken ct = default)
    {
        ...
        return await db.Employees.AsNoTracking()
            .Where(x => x.Id == id)
            .Select(x => new EmployeeSummary(x.Id, x.Username, x.FullName, x.IsAdmin, x.LastActivePositionId))
    ...
    public async Task<IReadOnlyList<LookupItem>> SearchPermissionGroupsAsync(string? search, CancellationToken ct = default)
    {
        var q = Normalize(search);
        await using var db = await dbFactory.CreateDbContextAsync(ct);
        return await db.PermissionGroups.AsNoTracking()
            .Where(x => q == "" || x.Name.Contains(q) || (x.Description != null && x.Description.Contains(q)))
            .OrderBy(x => x.Name)
            .Take(30)
            .Select(x => new LookupItem(x.Id, x.Name))
            .ToListAsync(ct);
    ...
    public async Task<PageResult<EmployeePageRow>> GetEmployeesAsync(int page, int pageSize, string? search, CancellationToken ct = default)
    {
        ...
        var items = await query
            ...
            .Select(x => new EmployeePageRow(
                ...
                x.IsActive,
                x.IsAdmin,
                x.Positions.Where(p => p.EndedAt == null && p.PositionId == x.LastActivePositionId)
    ...
    public async Task<PageResult<OrganizationPageRow>> GetOrganizationsAsync(int page, int pageSize, string? search, CancellationToken ct = default)
    {
        ...
        var items = await query
            ...
            .Skip((page - 1) * pageSize)
            .Take(pageSize)
            .Select(x => new OrganizationPageRow(x.Id, x.Name, x.IsActive, x.Employees.Count))
            .ToListAsync(ct);
        ...
        var query = db.Permissions.AsNoTracking()
            .Select(x => new
            {
                ...
                DirectAssignmentCount =
                    db.UserPositionPermissions.Count(up => up.PermissionId == x.Id) +
                    db.EmployeePermissions.Count(ep => ep.PermissionId == x.Id),
                GroupMembershipCount = db.PermissionGroupPermissions.Count(gp => gp.PermissionId == x.Id)
            });
    ...
    private static int Size(int size) => Math.Clamp(size, 10, 100);
