...
public sealed class AdminQueryService(IDbContextFactory<ApplicationDbContext> dbFactory)
{
    ...
    {
        page=Page(page);pageSize=Size(pageSize);var q=Normalize(search);
        await using var db=await dbFactory.CreateDbContextAsync(ct);
        var query=db.Employees.AsNoTracking().Where(x=>q==""||x.FullName.Contains(q)||x.Username.Contains(q));
        var total=await query.CountAsync(ct);

        // Resolve the organization name through OrganizationId instead of relying on the
        // Employee.Organization navigation. This keeps employees visible even if an older
        // database contains an orphaned/stale organization reference.
        var rows=await query.OrderBy(x=>x.FullName).Skip((page-1)*pageSize).Take(pageSize)
